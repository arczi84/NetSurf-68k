...
{	
	...
	set(obj->STR_fav_label, MUIA_String_Contents, strdup(ulubione2->nazwa));
	set(obj->STR_fav_url, MUIA_String_Contents, strdup(ulubione2->adres));
}
...
DoMethod (obj->Listview, MUIM_List_Redraw, MUIV_List_GetEntry_Active);
	...
	if (inum == 1 ) {	
		   ...
		   nsoption_charp(favourite_1_label) = strdup(nazwa);
		   }
	else if (inum == 2 ) {
		   nsoption_charp(favourite_2_url) = strdup(url);
		   nsoption_charp(favourite_2_label) = strdup(nazwa);	   	      
		   } 
	else if (inum == 3 ) {
		   nsoption_charp(favourite_3_url) = strdup(url);
		   nsoption_charp(favourite_3_label) = strdup(nazwa);	     
		   } 
	else if (inum == 4 ) {
		   nsoption_charp(favourite_4_url) = strdup(url);
		   nsoption_charp(favourite_4_label) = strdup(nazwa);	   	      
		   } 	
	else if (inum == 5 )  {
		   nsoption_charp(favourite_5_url) = strdup(url);
		   nsoption_charp(favourite_5_label) = strdup(nazwa);	   	   
		   } 
	else if (inum == 6  ) {
		   nsoption_charp(favourite_6_url) = strdup(url);
		   nsoption_charp(favourite_6_label) = strdup(nazwa);	   	   
		   } 
	else if (inum == 7  ) {
		   nsoption_charp(favourite_7_url) = strdup(url);
		   nsoption_charp(favourite_7_label) = strdup(nazwa);	   	     
		   } 	
	else if (inum == 8  ) {
		   nsoption_charp(favourite_8_url) = strdup(url);
		   nsoption_charp(favourite_8_label) = strdup(nazwa);    
		   }	 
	else if (inum == 9 ) {
		   nsoption_charp(favourite_9_url) = strdup(url);
		   nsoption_charp(favourite_9_label) = strdup(nazwa);	         
		   } 
	else if (inum == 10 ) {
		   nsoption_charp(favourite_10_url) = strdup(url);
		   nsoption_charp(favourite_10_label) = strdup(nazwa);	   	   		     
		   } 	
	else if (inum == 11 ) {
		   nsoption_charp(favourite_11_url) = strdup(url);
		   nsoption_charp(favourite_11_label) = strdup(nazwa);	   
		   } 
	else if (inum == 12 ) {
		   nsoption_charp(favourite_12_url) = strdup(url);
		   nsoption_charp(favourite_12_label) = strdup(nazwa);   
		}
