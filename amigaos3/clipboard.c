...
static void gui_get_clipboard(char **buffer, size_t *length)
{
	...
	*length = 0;
	...
	if (strlen(ReadClip()) > 0) {
		...
		*buffer = malloc(strlen(ReadClip()));//gui_clipboard.length);
		
		//if (*buffer != NULL)
		//	*buffer = strdup(ReadClip());
		
		if (ReadClip() != NULL) {
			memcpy(*buffer, ReadClip(),//gui_clipboard.buffer,
					strlen(ReadClip()));
			*length = strlen(ReadClip());	
		}
	}
}
		...
		nsclipboard_styles styles[], int n_styles)
{
	if (gui_clipboard.buffer_len < length + 1) {
		...
		new_buff = realloc(gui_clipboard.buffer, length );
		if (new_buff == NULL)
			return;

		gui_clipboard.buffer = new_buff;
		gui_clipboard.buffer_len = length ;
