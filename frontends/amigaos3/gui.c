...
static const __attribute__((used)) char *stack_cookie = "\0$STACK:1000000\0";
...
#include "amigaos3/fs_backing_store.h"
...
#ifdef RTG
#	define NSFB_TOOLBAR_DEFAULT_LAYOUT "blfrhuvaqetk123456789xwzgdmyop"
#else
...
#if defined  AGA || __clib2__ || NO_TIMER
...
//Printf(strndup(url,21));
	...
	if (strstr(mime_type, "video") != NULL)
			...
			Execute("echo \"ram:\" >RAM:mp3script",0,0); 
			strcpy(run, "echo \"C:wget ");	
			nodlpath = 1;
			}			
	...
	if (wget)
		...
		strcat(run," -Oram:video.mp4  \" > RAM:script"); //--no-check-certificate
		Execute(run, 0, 0);	
		...
		if  (strstr( url, "youtubeinmp3") != NULL)
			{
			strcpy(run, "run > nil: ");
			strcat(run, nsoption_charp(net_player));
			strcat(run, " RAM:video.mp4");
			Execute(run, 0, 0);
			}
		else
		...
		if (strstr(url, "www.modules.pl")!=NULL)
		...
		*/
		...
		{	
		Execute("delete > nil: ram:*.mp3", 0, 0);
		
		BPTR fh;
		fh = Open("CON:5/5/600/100", MODE_NEWFILE);
		
		strcat(run,"  \" >> RAM:mp3script");
		SetTaskPri(FindTask(0), -10);
		Execute(run, 0, 0);

		Execute("echo \"endcli\" >> ram:mp3script", 0, 0);			
		Execute("execute ram:mp3script", fh, 0);
		...
		Close(fh);
		//Execute("delete > nil: ram:mp3script", 0, 0);	
		}	
	else	
		{
		SetTaskPri(FindTask(0), -10);
...
main(int argc, char** argv)
{
	...
	if (nsoption_bool(bitmap_fonts))
		framebuffer_table.layout = framebuffer_layout_table_internal;
