...
text_input(fbtk_widget_t *widget, fbtk_callback_info *cbi)
{
	...
	LOG("(fbtk) got value %d", cbi->event->value.keycode);
	...
	case NSFB_KEY_MOUSE_5:               
 	case NSFB_KEY_LALT: /*Don't act on LALT and RALT*/
...
text_input_click(fbtk_widget_t *widget, fbtk_callback_info *cbi)
{
	...
	//if (nsoption_bool(bitmap_fonts))
		//fb_font_position_ttf = fb_font_position;
...
//	if (nsoption_bool(bitmap_fonts))
	//	fb_font_width_ttf = fb_font_width;
